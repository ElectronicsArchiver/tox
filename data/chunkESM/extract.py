...
def main(args):
    ...
    # Uppercase FASTA file
    with open(args.fasta_file) as f:
        lines = f.readlines()

    uppercase_lines = []

    print("Convert fasta file to uppercase")
    for line in lines:
        if(line[0] == '>'):
    ...
    output_file = str(str(args.fasta_file).split('.')[0] + "_uppercase" + ".fasta")

    print("Load uppercase fasta file")
    with open(output_file, 'w') as f:
        for item in uppercase_lines:
            f.write(item)

    dataset = FastaBatchedDataset.from_file(output_file)
